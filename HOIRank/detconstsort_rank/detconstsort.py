...
def infer_with_detconstsort(file, flip_choice, inferred=False, hidden=False, blind=False, seedy="no_seed"):
    filename = os.path.basename(settings["READ_FILE_SETTINGS"]["PATH"]).split('.')[0]
    test_data = pd.read_csv(gt, index_col=False)

    write_path = './HOIRank/Datasets/' + filename + '/Ranked/' + flip_choice + '/' + str(seedy) + '/DetConstSort_Ranked'
...
def DetConstSortHidden(flip_choice, seedy="no_seed"):
    """
    ...
    colorblind_ranked_gt = filter(find_unaware_ranked, rank.get_files(
        './HOIRank/Datasets/' + filename + '/Ranked/' + flip_choice + '/' + str(seedy) + '/Colorblind_Ranked'))
    ...
    for ranked in colorblind_ranked_gt:
        make_inferred_versions_gt_ranked(flip_choice, ranked, seedy)

    inferred_ranked = rank.get_files(
        './HOIRank/Datasets/' + filename + '/Transit/nonBlind/' + flip_choice + '/' + str(seedy) +
        '/Colorblind_Ranked_Inferred')

    for file in inferred_ranked:
        print(file)
        infer_with_detconstsort(file, flip_choice, inferred=True, hidden=True, seedy=seedy)


def make_inferred_versions_gt_ranked(flip_choice, path, seedy, blind=False):
    """
    ...
    if seedy == "no_seed":
        ...
        read_path = './HOIRank/Datasets/' + experiment_name + '/Inferred/' + flip_choice + '/' + str(seedy)
    test_read_path = './HOIRank/Datasets/' + experiment_name + '/Testing/'
    if blind:
        write_path = ('./HOIRank/Datasets/' + filename + '/Transit/Blind/' + flip_choice + '/' + str(seedy) +
                      '/Blind_Ranked_Inferred')
    else:
        write_path = ('./HOIRank/Datasets/' + filename + '/Transit/nonBlind/' + flip_choice + '/' + str(seedy)
                      + '/Colorblind_Ranked_Inferred')
    if not os.path.exists(write_path):
        os.makedirs(write_path)

    # this is the file to which you want to add inferred demographics
    background_file = pd.read_csv(path)
    df_file = background_file[['doc_id', 'Gender', 'judgement']]

    # these are the files with inferred demographics from simulations
...
def DetConstSortNotHidden(flip_choice, seedy="no_seed"):
    """
    ...
    inferred_ranked = filter(find_unaware_ranked, rank.get_files(
        './HOIRank/Datasets/' + filename + '/Ranked/' + flip_choice + '/' + str(seedy) + '/Inferred_Ranked'))
    ...
    if flip_choice == 'CaseStudies':
        ranked_gt = filter(find_unaware_ranked, rank.get_files(
            './HOIRank/Datasets/' + filename + '/Ranked/' + flip_choice + '/' + str(seedy) + '/GroundTruth_Ranked'))
    ...
    for file in inferred_ranked:
        print(file)
        infer_with_detconstsort(file, flip_choice, inferred=True, hidden=False, seedy=seedy)


def DetConstSortBlind(flip_choice, seedy="no_seed"):
    ...
    blind_ranked_gt = filter(find_unaware_ranked, rank.get_files(
        './HOIRank/Datasets/' + filename + '/Ranked/' + flip_choice + '/' + str(seedy) + '/BlindGroundTruth_Ranked'))

    # make inferred versions of the files including version with gt inserted(0% wrong inference)
    for ranked in blind_ranked_gt:
        make_inferred_versions_gt_ranked(flip_choice, ranked, seedy, blind=True)
    inferred_ranked = rank.get_files(
        './HOIRank/Datasets/' + filename + '/Transit/Blind/' + flip_choice + '/' + str(seedy) + '/Blind_Ranked_Inferred')
    for file in inferred_ranked:
        print(file)
        infer_with_detconstsort(file, flip_choice, inferred=True, hidden=False, blind=True, seedy=seedy)
