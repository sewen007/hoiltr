...
def GenerateFiles():
    ...
    for level in wrong_inference_levels:
        ...
        train_data[str(prot_attr) + " - " + str(level) + " percent wrong"  ] = select_and_assign(train_data[str(prot_attr)], level)

        gen_file = './FairRank/Datasets/' + experiment_name + '/Variant_Inferences' + '/Variant_Inferences_' + experiment_name + '_' + str(level) + '.csv'
