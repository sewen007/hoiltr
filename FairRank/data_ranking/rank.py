    ...
    for file in inferred:
        for model in models:
            ...
            if flip_choice == "CaseStudies":
                ...
                # rename Inferred_Gender column to Gender
            ...
            for index, row in result.iterrows():
                ...
                if int(row["doc_id"]) not in gt_inferred_combined_dict.keys():
                    gt_inferred_combined_dict[row["doc_id"]] = [str(int(row["doc_id"])),
                                                                ...
                                                                str(row["GT_score"]), str(int(row["Gender"]))]
                else:
...
def make_groundtruth_sex_uniform(path, flip_choice, sex=0):
    ...
    # store gt gender. We can't do this because it will affect ranking
    # background_file["GT_Gender"] = background_file["Gender"]
