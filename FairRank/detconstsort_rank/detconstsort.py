...
def make_inferred_versions_gt_ranked(flip_choice, path, blind=False):
    ...
    if blind:
        write_path = './FairRank/Datasets/' + filename + '/Transit/Blind/' + flip_choice + '/Blind_Ranked_Inferred'
    else:
        write_path = './FairRank/Datasets/' + filename + '/Transit/nonBlind/' + flip_choice + '/Colorblind_Ranked_Inferred'
    if not os.path.exists(write_path):
        os.makedirs(write_path)
